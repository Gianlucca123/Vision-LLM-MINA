...
The custom_evaluation folder consists of 4 directories and 1 file:

- data: A folder that contains all the data generated by the scripts, such as frames, videos, model results, and benchmark scores. It includes data that will be used, such as the ground truth.
- src: A folder containing the three main Python scripts.
- models: A folder containing all the Python scripts for the LLM models that will be used.
- utils: A folder with auxiliary Python scripts.
- requirements.txt: A file that lists the libraries needed to run the code.  

### Step 1: Generate Degraded Videos and Frames

First, navigate to the folder `custom_evaluation`:
```bash
cd path_to_custom_evaluation
```
...
```bash
python3 -m src.degrade_frame_videos --video_path cam-philippines-5min.mp4 --output_fps 1 --time 00:55 01:01 03:15 03:44 --video_quality 480p --video_modifications no_modification
```
